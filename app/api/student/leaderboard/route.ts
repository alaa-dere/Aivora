    ...
    const [rows] = await db.query<RowDataPacket[]>(
      `
        SELECT
          ...
          COALESCE(SUM(CASE
            WHEN s.startedAt >= DATE_SUB(CURDATE(), INTERVAL 6 DAY)
            THEN
              LEAST(
                GREATEST(
                  TIMESTAMPDIFF(
          ...
          COALESCE(SUM(CASE
            WHEN s.startedAt >= DATE_SUB(CURDATE(), INTERVAL 13 DAY)
        ...
        LEFT JOIN lesson_study_session s ON s.enrollmentId = e.id
