...
export default function TeacherProfileView({ apiFetch, theme, onProfileUpdated = () => {} }) {
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      setProfile(teacher);
      setFullName(String(teacher?.fullName || ''));
      setEmail(String(teacher?.email || ''));
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const nextTeacher = payload?.teacher || null;
      setProfile(nextTeacher);
      onProfileUpdated(nextTeacher);
      setCurrentPassword('');
      setNewPassword('');
      setConfirmPassword('');
  ...
  const uploadPhotoFromAsset = async (asset) => {
    ...
    try {
      ...
      const fileName = asset.fileName || `teacher-${Date.now()}.jpg`;
      ...
      const res = await apiFetch('/api/teacher/profile/photo', {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
    ...
    <View style={portalStyles.adminWrap}>
        ...
        <ScrollView>
          <View style={[portalStyles.adminSection, { backgroundColor: theme.cardBg, borderColor: theme.cardBorder }]}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              <Pressable
                onPress={() => (profileImageSource ? setPhotoViewerOpen(true) : setPhotoOptionsOpen(true))}
                style={{
                  width: 50,
                  height: 50,
                  borderRadius: 999,
              ...
              >
                {profileImageSource ? (
                  <Image source={profileImageSource} style={{ width: '100%', height: '100%' }} resizeMode="cover" />
                ) : (
                  <Text style={{ color: '#1d4ed8', fontWeight: '800' }}>{initials}</Text>
                )}
              </Pressable>
              <View style={{ flex: 1 }}>
          ...
          <View style={[portalStyles.adminSection, { backgroundColor: theme.cardBg, borderColor: theme.cardBorder }]}>
            ...
            >
              <Text style={[portalStyles.secondaryBtnText, { color: '#ffffff', fontWeight: '800' }]}>
      ...
      <Modal visible={photoViewerOpen} transparent animationType="fade" onRequestClose={() => setPhotoViewerOpen(false)}>
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.88)', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          {profileImageSource ? (
            <Image source={profileImageSource} style={{ width: '100%', height: 320, borderRadius: 16 }} resizeMode="contain" />
          ) : null}
